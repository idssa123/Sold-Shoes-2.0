    ...
    <header style={{ background: "var(--bg2)", borderBottom: "1px solid var(--bdr)" }} className="sticky top-0 z-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 flex h-14 items-center gap-4">
        ...
        <form onSubmit={e => { e.preventDefault(); if (q.trim()) { navigate(`/products?q=${encodeURIComponent(q.trim())}`); setQ(""); } }}
          className="flex-1 max-w-sm mx-2 hidden sm:block">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-auto">
          ...
          {user ? (
            <div className="flex items-center gap-2">
              {}
              {favCount > 0 && (
                <Link to="/favorites" className="relative h-9 w-9 rounded-full flex items-center justify-center transition-all"
          ...
          {}
          <Link to="/cart" className="relative h-9 w-9 rounded-full flex items-center justify-center transition-all"
